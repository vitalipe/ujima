...
(deftest window-facts-flatten-tiled-and-floating-with-workspace
  (let [tree {:nodes [{:type "workspace" :name "write"
                      ...
                      {:type "workspace" :name "1"
                       :nodes [{:id 5 :nodes []}]}]}]
    (is (= [{:con-id 9 :workspace "write" :focused? true  :floating? false :wtype "normal" :fullscreen? true  :class "libreoffice-writer" :title "Essay"}
            {:con-id 7 :workspace "write" :focused? false :floating? true  :wtype "dialog" :fullscreen? false :class nil :title "Tip"}]
...
(deftest window-facts-from-real-i3-json
  (let [line (str "{\"nodes\":[{\"type\":\"workspace\",\"name\":\"web\",\"nodes\":"
    ...
    (is (= [{:con-id 42 :workspace "web" :focused? true :floating? false :wtype "normal" :fullscreen? false :class "ujima-web" :title "Books"}]
